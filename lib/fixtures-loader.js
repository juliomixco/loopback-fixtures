...
module.exports = {
  savedData: {},
  loadFixtures: function(models, fixturesPath, opt) {
    ...
    _.each(fixtures, (function(_this) {
      return function(fixture) {
        ...
        return loadingFixturesPromises.push(_this.loadYamlFixture(models, fixtureData, opt));
      };
    })(this));
    return Promise.all(loadingFixturesPromises);
  },
  purgeDatabase: function(models, opt) {
    var purgeModelPromises;
    purgeModelPromises = [];
    _.forEach(models, (function(_this) {
      return function(model) {
        return purgeModelPromises.push(_this.purgeModel(model, opt));
      };
    })(this));
    return Promise.all(purgeModelPromises);
  },
  purgeModel: function(model, opt) {
    return new Promise(function(resolve, reject) {
      return model.destroyAll({}, opt, function(err) {
        if (err) {
          reject(err);
        }
        return resolve();
  ...
  loadYamlFixture: function(models, fixtureData, opt) {
      ...
      return function(modelData) {
        ...
        return Promise.each(modelFixtures, function(fixture) {
          return _this.replaceReferenceInObjects(fixture.object).then(function(object) {
            return models[modelData.name].create(object, opt);
